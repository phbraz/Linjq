...
public class InMemoryQueryProvider implements QueryProvider {
    ...
    private <T> Iterable<T> evaluateOrderBy(OrderByExpression<T, ?> expr) {
        ...
        if (expr.descending()) {
            return source.orderByDescending((io.linjq.functional.KeySelector) expr.keySelector());
        } else {
            return source.orderBy((io.linjq.functional.KeySelector) expr.keySelector());
        }
    }
    ...
    private <T> Iterable<T> evaluateThenBy(ThenByExpression<T, ?> expr) {
        ...
        if (expr.descending()) {
            return parentQueryable.orderByDescending((io.linjq.functional.KeySelector) expr.keySelector());
        } else {
            return parentQueryable.orderBy((io.linjq.functional.KeySelector) expr.keySelector());
        }
    }
}
